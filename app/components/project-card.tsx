      ...
      <CardContent className="p-4">
        <h3 className="font-bold text-lg md:text-xl mb-2">
          ...
          {productHuntRank && (
            <Link href={productHunt!} target="_blank" className="w-max flex">
              <h1 className="text-xs dark:text-yellow-400 hover:underline w-max">
        ...
        <div className="flex flex-wrap gap-2">
          {tags?.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center rounded-xl bg-white/10 px-2 py-1 text-[0.7rem] md:text-xs font-medium ring-1 ring-inset ring-gray-500/10"
            >
              {tag}
            </span>
          ))}
